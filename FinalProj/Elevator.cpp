...
void Elevator::tick(int currentTime)
{
    ...
    if (targetFloor < currentFloor && currentTime % TICKS_PER_ELEVATOR_MOVE == 0 && servicing)
    {
        currentFloor--;
    }
    else if (targetFloor > currentFloor && currentTime % TICKS_PER_ELEVATOR_MOVE == 0 && servicing)
    {
        currentFloor++;
    }
