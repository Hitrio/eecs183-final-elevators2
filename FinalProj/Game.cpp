...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    for (int i = 0; i < pickupList.size(); i++) {
        int personAtIndexI = pickupList[i] - '0';
        bool directionUpI = false;
        int currentI = building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(personAtIndexI).getTargetFloor() - pickupFloorNum;
        if (currentI > 0) {
            directionUpI = true;
        }
        for (int j = 0; j < pickupList.size(); j++) {
            bool directionUpJ = false;
            int personAtIndexJ = pickupList[j] - '0';
            int currentJ = building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(personAtIndexJ).getTargetFloor() - pickupFloorNum;
            if (currentJ > 0) {
                directionUpJ = true;
            }
            if (directionUpI != directionUpJ) {
                return false;
            }
            if ((i != j) && (pickupList.at(i) == pickupList.at(j))) {
                return false;
            }
        }
        if (personAtIndexI < 0) {
